...
def face_compare_tree(feat, user_data, tree):
    ...
    if sim>0.6:
        return {'id': user_data[face_id, 0][0], 'fullname': user_data[face_id, 1][0], 'Sim': sim, 'position':user_data[face_id, 2][0]}
    else:
        return {'id': None, 'fullname': 'uknown', 'Sim': 0, 'position': None}
...
class ArcFaceONNX:
    ...
    def face_compare(self, feet, employees_data, threshold=0.5):
        max_sim = -1
        info = {'fullname': 'uknown', 'Sim': max_sim, 'code': None, 'position': None}
        if employees_data is not None:
            for data in employees_data:
                feet_compare = data['feat']
